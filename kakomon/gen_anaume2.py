...
# ── 語彙を集める（誤答の素材）──────────────────────
# 主体（行為をする側）の判定。基金・機構・連合会も主体なので、
# これを取りこぼすと「義務／期限／認可／基金」のように
# 主語になれる語が1つだけになり、読まずに当てられる。
ACTOR_RE = re.compile(r"(大臣|署長|所長|局長|知事|機構|協会|組合|連合会|基金|市町村|都道府県|"
                      r"審査会|審査官|委員会|事業主|保険者|政府|国|会社|事業者|使用者|労働者|"
                      r"被保険者|加入者|加入員|受給権者|受給資格者|銀行|事務組合)$")

def cat(w):
    t = NOSP(w)
    if re.fullmatch(r"[0-9０-９]+(年|月|日|週間|時間|歳|人|円|％|か月|分の[0-9]+)?", t): return "数値"
    if ACTOR_RE.search(t): return "主体"
...
STOP = {"事項","場合","法律","前項","規定","必要","同項","各号","以下","当該","その他",
        "費用","診断","理由","目的","内容","額","者","もの","事業","業務","作業","区分",
        "手続","申請","結果","状況","措置","事務","期日","期限","名称","住所","氏名",
...
for u in units:
    ...
    for w in words:
        ...
        # 「日本銀行（本店、支店、代理店及び歳入代理店をいう。）」のように、
        # 直後の括弧書きが答えの説明になっていると読まずに当てられる。
        if re.match(r"\s*[（(]", t[p+len(w):p+len(w)+2]): continue
