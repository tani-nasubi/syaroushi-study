...
DUTY  = [("しなければならない","することができる"),("することができる","しなければならない"),
         ("してはならない","することができる"),("努めなければならない","しなければならない")]
...
def mutate(text):
    ...
    for before, after in DUTY:
        if text.count(before) == 1:
            p = text.index(before)
            if not in_quote(text, p): cands.append(("義務と裁量", before, after))
    # 数値はしきい値だけを対象にする。
...
for subj in SUBJ:
    ...
    def build_wrong(src):
        ...
        ai = next(i for i, x in enumerate(items) if x["bad"])
    ...
    def build_right(correct, wrongs):
        ...
        random.shuffle(items)
        ai = next(i for i, x in enumerate(items) if x["good"])
    ...
    while len(qs) < TARGET and mutables:
        idx += 1
        if idx > len(mutables) * 10: break
        if len(qs) % 2 == 0:
            src = mutables[idx % len(mutables)]
            key = NOSP(src["t"])[:40]
            if used_ans[key] >= 4: continue
            b2 = build_wrong(src)
            if not b2: continue
            used_ans[key] += 1; emit(b2)
        else:
            correct = pool[idx % len(pool)]
            ck = NOSP(correct["t"])[:40]
            if used_ans[ck] >= 4: continue
            pickable = [m for m in mutables
                        if NOSP(m["t"])[:40] != ck and used_dis[NOSP(m["t"])[:40]] < 10]
            if len(pickable) < 4: continue
            random.shuffle(pickable)
            ws = sorted(pickable, key=lambda m: used_dis[NOSP(m["t"])[:40]])[:4]
            used_ans[ck] += 1
            for w in ws: used_dis[NOSP(w["t"])[:40]] += 1
            emit(build_right(correct, ws))
